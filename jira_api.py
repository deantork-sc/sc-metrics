...
class JiraApi:
    ...
    def demo(self):
        ...
        with open('demo/changelog.json', 'w') as outfile:
            json.dump(json.loads(changelog), outfile)

        group = self.get_group().text
        with open('demo/group.json', 'w') as outfile:
        ...
        with open('demo/search.json', 'w') as outfile:
            json.dump(json.loads(search), outfile)

        issue = self.get_issue("HLP-3335").text
        with open('demo/issue.json', 'w') as outfile:
